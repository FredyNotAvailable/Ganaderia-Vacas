...
class SupabaseGanaderiaRepository {
    ...
    async getByUserId(userId) {
        ...
        const { data: profile } = await supabase_client_1.supabase
            .from('perfiles')
            .select('rol_sistema')
            .eq('user_id', userId)
            .single();
        if (profile?.rol_sistema === 'SUPERADMIN') {
            const { data: allGanaderias, error: allError } = await supabase_client_1.supabase
                .from(this.table)
                .select('*');
            if (allError)
            ...
            return (allGanaderias || []).map(g => ({
                ...
                permiso: permisos_1.PERMISOS.ELIMINAR,
                rol_detalle: { nombre: 'Super Admin', codigo: 'SUPERADMIN' }
            }));
        }
        // 1. Fetch owned ganaderias
        const { data: owned, error: ownedError } = await supabase_client_1.supabase
            .from(this.table)
            .select('*')
        ...
        const { data: shared, error: sharedError } = await supabase_client_1.supabase
        ...
        const sharedNormalized = (shared || [])
            ...
            return {
                ...ganaderia,
                rol: 'COMPARTIDA',
                permiso: maxPermisoCodigo || permisos_1.PERMISOS.LECTURA, // Fallback safe
                rol_detalle: rol ? {
                    id: s.rol_id,
                    nombre: rol.nombre,
                    codigo: rol.codigo
    ...
    async create(data) {
        const { data: newGanaderia, error } = await supabase_client_1.supabase
            .from(this.table)
            .insert(data)
            .select()
            .single();
