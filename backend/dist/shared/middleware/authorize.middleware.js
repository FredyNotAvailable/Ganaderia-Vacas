...
const authorize = (permisoRequerido) => {
    return async (req, res, next) => {
        try {
            ...
            let ganaderiaId = req.query.ganaderia_id || req.body.ganaderia_id || req.params.ganaderia_id;
            ...
            if (!ganaderiaId && req.params.id) {
                ...
                if (table) {
                    const { data } = await supabase_client_1.supabase
                        .from(table)
                        .select('ganaderia_id')
                        .eq(`${table.slice(0, -1)}_id`, req.params.id)
                        .single();
                    if (data)
                        ganaderiaId = data.ganaderia_id;
                }
            }
            if (!ganaderiaId) {
            ...
            const hasPerm = await permission_service_1.PermissionService.hasPermission(userId, ganaderiaId, permisoRequerido);
            if (!hasPerm) {
